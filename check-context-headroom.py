...
def _build_nudge_output(
    *,
    ...
    nudge = (
        ...
        "message normally.\n"
        "\n"
        "ESCAPE HATCH (inside /save-context): the skill's Step 0 asks\n"
        "'is anything material worth saving?'. If you genuinely determine\n"
        "the answer is 'nothing notable' (a quick Q&A, a read-only chat,\n"
        "a throwaway debug loop), short-circuit there and Step 5 will\n"
        "still run, marking the session saved so the hook stops nudging.\n"
        "DO NOT use the escape to dodge work - real learnings, surprises,\n"
        "or user feedback always warrant a save.\n"
        "\n"
        "DO NOT DEFER. This is session-critical and the hook WILL continue\n"
        "to re-fire on every prompt until /save-context runs (whether it\n"
        "writes memories or short-circuits at Step 0)."
    )
    output = {
        "hookSpecificOutput": {
